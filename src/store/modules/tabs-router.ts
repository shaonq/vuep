...
const ignoreCacheRoutes: string[] = []
...
export const useTabsRouterStore = defineStore('tabsRouter', {
  ...
  actions: {
    ...
    subtractCurrentTabRouter(newRoute: TRouterInfo) {
      ...
      this.tabRouterList = this.tabRouterList.slice(0, routeIdx).concat(this.tabRouterList.slice((routeIdx as number) + 1))
    },
    // 处理关闭右侧
    subtractTabRouterBehind(newRoute: TRouterInfo) {
      const { routeIdx } = newRoute
      this.tabRouterList = this.tabRouterList.slice(0, (routeIdx as number) + 1)
    },
    // 处理关闭左侧
    ...
    subtractTabRouterOther(newRoute: TRouterInfo) {
      const { routeIdx } = newRoute
      this.tabRouterList = homeRoute.concat([this.tabRouterList?.[routeIdx as number]])
    },
    removeTabRouterList() {
      this.tabRouterList = []
    },
