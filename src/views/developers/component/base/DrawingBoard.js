...
export default class DrawingBoard {
  ...
  constructor({ canvas, width = canvas.scrollWidth, height = canvas.scrollHeight, color = '#fc4639', backgroundImage, lineWidth }) {
    ...
    const points = [],
      lazy = new LazyBrush({ radius: 5, enabled: true, initialPoint: { x: 0, y: 0 } })

    // 画笔移动 使用LazyBrush算法
    const moveTo = (x, y) => {
      x = this.dpiSize(x)
      y = this.dpiSize(y)
      const hasChanged = lazy.update({ x, y }) // , { both: true }
      ...
      if (lazy.brushHasMoved() || isDisabled) {
        ...
        for (let i = 1, len = points.length; i < len; i++) {
          ...
          const midPoint = middlePoint(p1, p2)
    ...
    const [mousemove, mousedown, mouseup] = [isMobile ? 'touchmove' : 'mousemove', isMobile ? 'touchstart' : 'mousedown', isMobile ? 'touchend' : 'mouseup']
  ...
  clearCanvas(closeTempData) {
    ...
    if (this.backgroundImage) {
      const zoom = this.canvas.width / this.backgroundImage.width
      this.backgroundImage && this.ctx.drawImage(this.backgroundImage, 0, 0, this.backgroundImage.width * zoom, this.backgroundImage.height * zoom)
    }
  }
  putImgData(imgData) {
