export default {
  ...
  toDate: function (q) {
    if (q.length === 19) {
      const p = q.substring(0, 4)
      const r = q.substring(5, 7)
      const m = q.substring(8, 10)
      const l = q.substring(11, 13)
      const n = q.substring(14, 16)
      const o = q.substring(17, 19)
  ...
  addMonths: function (p, o, f) {
    const l = arguments.length === 1 ? this.toDate(this.today()) : this.toDate(o)
    const m = l.getMonth()
    const n = l.getDate()
    const q = this.getMonthDays(l, l.getMonth() + parseInt(p))
    if (n > q) {
      l.setDate(q)
    }
  ...
  addYears: function (m, n, f) {
    const l = arguments.length === 1 ? this.toDate(this.today()) : this.toDate(n)
  ...
  firstDayOfMonth: function (m, f) {
    const l = arguments.length === 0 ? this.toDate(this.today()) : this.toDate(m)
    l.setDate(1)
    return this.toString(l, f)
  },
